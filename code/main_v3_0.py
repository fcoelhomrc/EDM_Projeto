...
def get_measurement(tag):
    ...
    if tag == measure_timer:
        ...
        if filter_index == 0: # Clear photodiode
            ...
            if CALIBRATED:
                x = measurement/cache["clear"]
                diff = white_calib_cache[filters[filter_index]] - dark_calib_cache[filters[filter_index]]
                cache[filters[filter_index]] = 255*(x - dark_calib_cache[filters[filter_index]])/diff # Convert to RGB scale
            else:
        ...
        if filter_index == 0: # Clear photodiode
            current_cache[filters[filter_index]] = measurement # Store new clear value
        else: # RGB photodiodes
            current_cache[filters[filter_index]] = (measurement/current_cache["clear"])*255 # Convert to RGB scale
